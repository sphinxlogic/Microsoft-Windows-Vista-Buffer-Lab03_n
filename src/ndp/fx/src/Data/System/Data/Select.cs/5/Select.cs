            ...
            if (index == null) {
                if (nCandidates == 0) { 
                    ...
                    for (i=0; i < lenIndexDesc; i++) { 
                        ColumnInfo candidateColumn = candidateColumns[DataKey.ColumnOrder(indexDesc[i])];
                    ...
                    if (equalsOperator) { 
                        ...
                        IndexField[] ndxFields = DataTable.zeroIndexField; 
                        if (0 < ndxDesc.Length) {
                            ndxFields = new IndexField[ndxDesc.Length];
                            for(int count = 0; count < ndxDesc.Length; count++) {
                                DataColumn col = table.Columns[DataKey.ColumnOrder(ndxDesc[count])]; 
                                bool sortDesc = DataKey.SortDecending(ndxDesc[count]);
                                ndxFields[count] =  new IndexField(col, sortDesc); 
                            } 
                        }
                        index = new Index(table, ndxDesc, ndxFields, recordStates, null); 
