...
namespace System.Data.Common {
    ...
    public 
#endif
    abstract class DBDataPermission :  CodeAccessPermission, IUnrestrictedPermission { 
#endif 
        ...
        protected
#endif 
        void Clear() { // V1.2.3300, MDAC 83105
            _keyvaluetree = null;
            _keyvalues = null;
        } 
...
#if ORACLE 
            OraclePermission operand = (OraclePermission) target;
#else 
...
#if ORACLE 
            OraclePermission newPermission = (OraclePermission) operand.Copy(); 
#else
        ...
        override public bool IsSubsetOf(IPermission target) {
            ...
            if (null != superset._keyvalues) {
                foreach(DBConnectionString constr in superset._keyvalues) { 
                    Debug.WriteLine("+ " + constr.UsersConnectionString(false)); 
                }
            } else Debug.WriteLine("+ <>"); 
            if (null != _keyvalues) {
                foreach(DBConnectionString constr in _keyvalues) {
                    Debug.WriteLine("- " + constr.UsersConnectionString(false));
                } 
            } else Debug.WriteLine("- <>");
#endif 
