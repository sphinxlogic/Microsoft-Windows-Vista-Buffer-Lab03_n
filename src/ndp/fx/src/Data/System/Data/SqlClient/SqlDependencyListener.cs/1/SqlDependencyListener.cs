    ...
    private class SqlConnectionContainer { 
        ...
        internal SqlConnectionContainer(SqlConnectionContainerHashHelper hashHelper, string appDomainKey, bool useDefaults) {
            ...
            try { 
                ...
                ((SqlConnectionString) _con.ConnectionOptions).CreatePermissionSet().Assert();
                _con.Open(); 
                ...
                try {
                    // Connection Open() and other setup/allocations done outside of try/catch - TearDownAndDispose()
                    // should not be run if above fails. 
            ...
            try {
                ...
                try { 
                    ...
                    if (!restart) { 
                        ...
                        try {
                            com.ExecuteNonQuery(); // Cannot add 'IF OBJECT_ID' to create procedure query - wrap and `swallow.
                        } 
                        catch (Exception e) {
                            if (!ADP.IsCatchableExceptionType(e)) { 
                                throw; 
                            ...
                            try { // Since the failure will result in a rollback, rollback our object.
                                ...
                                ADP.TraceExceptionWithoutRethrow(f); // `Swallow failure, but trace for now.
                            } 
                        }
                    ...
                    com.CommandText = 
                          ...
                          + " IF (SELECT COUNT(*) FROM sys.services WHERE NAME='"+_queue+"') = 0" 
                             ...
                             + " IF (SELECT COUNT(*) FROM sys.database_principals WHERE name='sql_dependency_subscriber' AND type='R') <> 0"
                                  ...
                                  + " END; " 
                                + " END;" 
                          + " BEGIN DIALOG @dialog_handle FROM SERVICE "+_escapedQueueName+" TO SERVICE '"+_queue+"'";
                    ...
                    if (!_stop) {
                        while (reader.Read()) {
                            ...
                            for (int i=0; i<reader.FieldCount; i++) {
                                Bid.NotificationsTrace("<sc.SqlConnectionContainer.ProcessNotificationResults|DEP> column: " + reader.GetName(i) + ", value: " + reader.GetValue(i).ToString() + "\n"); 
                            } 
#endif
                                    ...
                                    if (null != notification) {
                                        ...
                                        if (index != 0) { // Ensure ';' present. 
                                            ...
                                            SqlDependencyPerAppDomainDispatcher dispatcher =
                                              (SqlDependencyPerAppDomainDispatcher) _staticInstance._sqlDependencyPerAppDomainDispatchers[result]; 
                                            if (null != dispatcher) {
                                                try {
                                                    ...
                                                    ADP.TraceExceptionWithoutRethrow(e); // `Swallow failure.  User event could throw exception. 
                                                }
                                            }
                                            else {
            ...
            try {
                try { 
                    lock (this) { 
                        if (!_stop) { // Only execute if we are still in running state.
                            try { 
                                ...
                                ADP.TraceExceptionWithoutRethrow(e); // `Swallow close failure, if it occurs.  Only trace it. 
                            }
                        } 
                    }
                    ...
                    try {
                        ...
                        ADP.TraceExceptionWithoutRethrow(f); // `Swallow exception from Invalidate.  User events can throw.
                    } 
                    ...
                    try { 
                        ...
                        ADP.TraceExceptionWithoutRethrow(f); // `Swallow close failure, if it occurs.  Only trace it. 
                    }
                ...
                if (0 == result) { // If we've reached refCount 0, destroy.
                    ...
                    lock (this) {
                        try {
                            ...
                            // Rather than fighting the race, just call it and `swallow.
                            ...
                            ADP.TraceExceptionWithoutRethrow(e); // `Swallow failure, if it should occur.
                        } 
                        _stop = true;
                    } 
 
                    // Need to loop until stopped and service & queue dropped.
                    // 
                    while (true) {
                        lock (this) {
                            if (_stopped) {
                            ...
                            if (_errorState) { // Exit if in error state! 
                        ...
                        Thread.Sleep(0); // Yield the thread since the stop has not yet completed. 
                    }
                } 
                else {
                        ...
                        if (ConnectionState.Closed != _con.State && ConnectionState.Broken != _con.State) { 
                            if (_com.Parameters.Count > 1) { // Need to close dialog before completing.
                                ...
                                try { 
                                    ...
                                    ADP.TraceExceptionWithoutRethrow(e); // `Swallow failure. 
                                } 
                            }
                ...
                if ( (XmlNodeType.Element == xmlReader.NodeType) && 
                    ...
                    while ((MessageAttributes.All != messageAttributes) && (xmlReader.MoveToNextAttribute())) {
                        try {
                            ...
                            ADP.TraceExceptionWithoutRethrow(e); // `Swallow failure, but trace. 
            ...
            lock (_sqlDependencyPerAppDomainDispatchers) {
                ...
                foreach (KeyValuePair<string, SqlDependencyPerAppDomainDispatcher> entry in _sqlDependencyPerAppDomainDispatchers) {
                    ...
                    try { 
                        ...
                        ADP.TraceExceptionWithoutRethrow(f); // `Swallow failure, but trace.
                    }
                }
            } 
        }
