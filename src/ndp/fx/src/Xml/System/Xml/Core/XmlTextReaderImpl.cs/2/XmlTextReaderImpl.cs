        ...
        private void AddDefaultAttributesAndNormalize() {
            ...
            if ( defaultAttributes != null ) {
                ...
                if (attrCount >= MaxAttrDuplWalkCount) {
                    ...
                    Array.Sort(nameSortedAttributes, SchemaAttDefToNodeDataComparer.Instance); 
                } 

                for (int i = 0; i < defaultAttributes.Length; i++) 
                {
                    SchemaAttDef attrDef = defaultAttributes[i];
                    if (AddDefaultAttribute(attrDef, true, nameSortedAttributes)) {
                        if (DtdValidation && standalone && attrDef.IsDeclaredInExternal) { 
...
//
        ...
        internal bool XmlValidatingReaderCompatibilityMode {
            set {
                ...
                // Bug fix for VSWhidbey 516556; These namespaces must be added to the nametable for back compat reasons. 
                if ( value ) { 
        ...
        private bool AddDefaultAttribute(SchemaAttDef attrDef, bool definedInDtd, NodeData[] nameSortedNodeData) {
            ...
            if (definedInDtd && nameSortedNodeData != null) { 
                // Note1: definedInDtd must be true because check looks different for Xsd documents:
                //        namespaces should be checked if prefixes are different. 
                // Note2: localName and prefix from attrDef may not be from nameTable if dealing with Xsd. 
                //        Comparer doesn't rely on this, but it is a bit faster if condition above is true.
        ...
        void RegisterConsumedCharacters(long characters, bool inEntityReference) {
            ...
            if (maxCharactersInDocument > 0) {
                ...
                if (newCharactersInDocument < charactersInDocument) {
                    ...
                    ThrowWithoutLineInfo(Res.XmlSerializeErrorDetails, new string[] { "MaxCharactersInDocument", "" });
                } 
                else { 
            ...
            if (maxCharactersFromEntities > 0 && inEntityReference) { 
                ...
                if (newCharactersFromEntities < charactersFromEntities) { 
                    // Integer overflow while counting
                    ThrowWithoutLineInfo(Res.XmlSerializeErrorDetails, new string[] { "MaxCharactersFromEntities", "" });
                }
                else { 
