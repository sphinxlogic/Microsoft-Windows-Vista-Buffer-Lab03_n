...
namespace System.Web {
    ...
    public sealed class HttpCachePolicy { 
        ...
        void UpdateFromDependencies(HttpResponse response) {
            ...
            CacheDependency virtualDependency; 
            ...
            dependency = response.GetCacheDependency(); 
            virtualDependency = response.GetVirtualPathDependency();

            if (fileDependencies == null && cacheItemDependencies == null && dependency == null && virtualDependency == null) {
                return; 
            }
            ...
            if (fileDependencies != null) {
                ...
                foreach (string filename in fileDependencies) { 
                    fad = fmon.GetFileAttributes(filename);
                    if (fad == null) { 
                        fad = FileAttributesData.NonExistantAttributesData;
                    }

                    if (utcFileLastModifiedMax < fad.UtcLastWriteTime) { 
                        utcFileLastModifiedMax = fad.UtcLastWriteTime;
                    } 
 
                    if (_generateEtagFromFiles) {
                        sb.Append(filename); 
                        sb.Append(fad.UtcLastWriteTime.Ticks.ToString(CultureInfo.InvariantCulture));
            ...
            if (cacheItemDependencies != null) {
                ...
                foreach (string cacheKey in cacheItemDependencies) {
                    ...
                    if (entry != null) {
                        ...
                        if (_generateEtagFromFiles) { 
                            sb.Append(cacheKey);
                            sb.Append(utcCreated.Ticks.ToString(CultureInfo.InvariantCulture));
                        }
                    } 
                }
            } 
            ...
            if (dependency != null) {
                if (utcFileLastModifiedMax < dependency.UtcLastModified) { 
                    utcFileLastModifiedMax = dependency.UtcLastModified;
                }
            ...
            if (virtualDependency != null) { 
                if (utcFileLastModifiedMax < virtualDependency.UtcLastModified) {
                    utcFileLastModifiedMax = virtualDependency.UtcLastModified; 
                } 

                if (_generateEtagFromFiles) { 
                    string id = virtualDependency.GetUniqueID();
                    if (id == null) {
                        throw new HttpException(SR.GetString(SR.No_UniqueId_Cache_Dependency));
                    } 
                    sb.Append(id);
            ...
            if (_generateEtagFromFiles) { 
                ...
                _etag = MachineKeySection.HashAndBase64EncodeString(sb.ToString());
