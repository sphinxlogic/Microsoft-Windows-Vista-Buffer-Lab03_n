    ...
    sealed class FileChangesMonitor {
        ...
        DirectoryMonitor ListenToSubdirectoryChanges(string dirRoot, string dirToListenTo) { 
            ...
            if (IsBeneathAppPathInternal(dirRootSubDir)) { 
                ...
                dirMonSubDir.StartMonitoringFile(dirToListenTo, new FileChangeEventHandler(this.OnCriticaldirChange), dirRootSubDir);
            } 
            else if (Directory.Exists(dirRootSubDir)) {
                dirMonSubDir = new DirectoryMonitor(dirRootSubDir, true, UnsafeNativeMethods.RDCW_FILTER_FILE_CHANGES); 
                try { 
                    dirMonSubDir.StartMonitoringFile(null, new FileChangeEventHandler(this.OnCriticaldirChange), dirRootSubDir);
                } 
                catch {
                ...
                if (dirMonSubDir == null) { 
                    dirMonSubDir = new DirectoryMonitor(dirRoot, false, UnsafeNativeMethods.RDCW_FILTER_FILE_AND_DIR_CHANGES);
