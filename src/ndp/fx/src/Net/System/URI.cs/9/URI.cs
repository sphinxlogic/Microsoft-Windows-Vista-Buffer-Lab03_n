...
namespace System { 
    ...
    public partial class Uri : ISerializable { 
        ...
        private static bool s_ConfigInitialized;                                     // Have the config values been 
                                                                                    // initalized from config file
        private static UriIdnScope s_IdnScope = UriIdnScope.None;                   // Value from config Uri section 
        private static bool s_IriParsing = false;                                   // Value from config Uri section 
        ...
        // internal IDN config state with additional Invalid & NotFound states 
        private enum IdnScopeFromConfig {
            None = 0, 
            AllExceptIntranet, 
            All,
            Invalid = Int32.MaxValue - 1, 
            NotFound = Int32.MaxValue
        }

        // internal IRI config state with additional Invalid & NotFound states 
        ...
        //
        // GetConfig - get the Uri config state 
        //
        // This function originally just used System.Config to get the 
        // new-to-Orcas Uri config section.  Unfortunately that created a 
        // circular dependency on System.Config that ultimately could cause
        // ConfigurationExceptions that didn't used to happen in Whidbey and 
        // thus break existing deployed applications.
        //
        // Now this function will determine if it is running in a client
        // application or a web scenario (ASP.NET).   If in a web scenario 
        // this code must still use System.Config to read config as the web
        // scenario has a hierarchy of config files that only System.Config 
        // can practically discover and pars.  If in a client scenario this 
        // code will now use System.Xml.XmlReader to parse machine and app
        // config files. 
        //
        // The default output of this function if it encounters invalid or
        // non-existent Uri config is the original Whidbey settings (no IDN
        // support and no IRI support). 
        //
        private static void GetConfig(ref UriIdnScope idnScope, ref bool iriParsing) { 
            if (IsWebConfig()) { 
                // This is a web scenario.
                // It is safe and *necessary* to use System.Config. 
                try {
                   UriSectionInternal uriSection = UriSectionInternal.GetSection();
                   if (uriSection != null) {
                       idnScope = uriSection.Idn; 
                       iriParsing = uriSection.IriParsing;
                   } 
                } 
                catch (ConfigurationException) {
                   // Simply ignore any ConfigurationException. 
                   // Throwing it would potentially break applications.
                   // Uri did not read config in previous releases.
                }
 
            } else {
                // This is a client application scenario. 
                // It is not safe to use System.Config for app compat reasons. 

                // Get the paths of the application and machine config files. 
                string runtimeDir = null;
                string appConfigFile = null;

                // Must Assert unrestricted FileIOPermission to get the app and machine config paths. 
                ...
                string machineConfigFile = Path.Combine(Path.Combine(runtimeDir, "Config"), "machine.config"); 

                IdnScopeFromConfig idnFromMachine, idnFromApp; 
                IriParsingFromConfig iriFromMachine, iriFromApp; 

                // Attempt tp parse machine and app config files if they exist. 
                ParseConfigFile(machineConfigFile, out idnFromMachine, out iriFromMachine);
                ParseConfigFile(appConfigFile, out idnFromApp, out iriFromApp);

                // App config takes precedence over machine config 
                // only if found and valid.
                switch (idnFromApp) { 
                    ...
                    case IdnScopeFromConfig.All: 
                        ...
                        break; 
                    default: // invalid or notfound 
                        switch (idnFromMachine) {
                            case IdnScopeFromConfig.None: 
                                idnScope = UriIdnScope.None;
                                break;
                            case IdnScopeFromConfig.AllExceptIntranet:
        ...
        //
        private static bool IsWebConfig() { 
            // Existence of string object associated with .appVPath is the key. 
        ...
        // 
        // Attempt to parse out the IDN and IRI settings in an xml config file.
        // 
        private static void ParseConfigFile(
                                string file,
                                out IdnScopeFromConfig idnStateConfig,
                                out IriParsingFromConfig iriParsingConfig) { 

            // Start with default NotFound values. 
            ...
            try { 
                // Create a filestream to read the config file
                // We cannot allow XmlReader.Create to do it as it considers 
                // the input file string as a URI and calls our ctor 
                // potentially creating endless recursion if the file string
                // requires us to read config. 
                FileStream filestream = new FileStream(file, FileMode.Open, FileAccess.Read);

                // Close the filestream when done.
                using (filestream) { 
                    // Create an XML reader that ignores:
                    ...
                    using(reader) { 
                        ...
                        // Find configuration element - return if not found. 
                        if (!reader.ReadToFollowing("configuration")) {
                            return;
                        }
                        // Find uri element - return if not found 
                        if (!reader.ReadToFollowing("uri")) {
                            return; 
                        } 

                        // Loop reading nodes until we hit the uri EndElement 
                        while (true) {

                            // If an EndElement and is "uri" then stop reading
                            if (reader.NodeType == XmlNodeType.EndElement) { 
                                if (ConfigStringEqual(reader.Name, "uri")) {
                                    break; 
                                } 
                            }
                            // If an Element look for our two elements of interest: 
                            // "idn" or "iriParsing"
                            if (reader.NodeType == XmlNodeType.Element) {
                                string value;
                                if (ConfigStringEqual(reader.Name, "idn")) { 
                                    // Try to get the "enabled" attribute value
                                    value = reader.GetAttribute("enabled"); 
                                    if (value != null) { 
                                        // Look for "None" or "AllExceptIntranet" or "All"
                                        // Otherwise invalid 
                                        if (ConfigStringEqual(value, "None")) {
                                            idnStateConfig = IdnScopeFromConfig.None;
                                        } else if (ConfigStringEqual(value, "AllExceptIntranet")) {
                                            idnStateConfig = IdnScopeFromConfig.AllExceptIntranet; 
                                        } else if (ConfigStringEqual(value, "All")) {
                                            idnStateConfig = IdnScopeFromConfig.All; 
                                        } else { 
                                            idnStateConfig = IdnScopeFromConfig.Invalid;
                                        } 
                                    }
                                } else if (ConfigStringEqual(reader.Name, "iriParsing")) {
                                    // Try to get the "enabled" attribute value
                                    value = reader.GetAttribute("enabled"); 
                                    if (value != null) {
                                        // Look for "false" or "true" 
                                        // Otherwise invalid 
                                        if (ConfigStringEqual(value, "false")) {
                                            iriParsingConfig = IriParsingFromConfig.False; 
                                        } else if (ConfigStringEqual(value, "true")) {
                            ...
                            // Go to next node 
                            if(!reader.Read()) break;  // break out of loop on eof
                        }
                    }
                } 
            }
                ...
                // Throw only "fatal" exceptions for backward compat
                if (NclUtilities.IsFatal(exception)) 
                    throw;
            }
            finally {
                // Revert read permission assert for this file. 
                FileIOPermission.RevertAssert();
            } 
        } 

        // Simple function to do OrdinalIgnoreCase string comparisons 
        ...
        private string GetLocalPath(){ 
            ...
            if (IsUncOrDosPath) 
            {
                ...
                if (IsUncPath) 
                {
                    ...
                    UnescapeString( host, 0, host.Length, result, ref count, c_DummyChar, c_DummyChar, c_DummyChar,
                                    UnescapeMode.CopyOnly, m_Syntax, false, false);
                ...
                UnescapeString(m_String, start, m_Info.Offset.Query, result, ref count, c_DummyChar, c_DummyChar, c_DummyChar, mode, m_Syntax, true, false);
        ...
        public string DnsSafeHost {
            get { 
                ...
                { 
                    ...
                    int count = 0; 
                    UnescapeString(ret, 0, ret.Length, dest, ref count, c_DummyChar, c_DummyChar, c_DummyChar, UnescapeMode.CopyOnly, m_Syntax, false, false);
        ...
        public string[] Segments { 
            get {
                ...
                //[....] codereview
                //I did not optimize this method
...
*********/
        ...
        { 
            ...
            { 
                ...
                { 
                    string newHost = null; 
                    if (CheckAuthorityHelper(pHost, 0, (ushort)host.Length, ref err, ref flags, m_Syntax, ref newHost) != (ushort)host.Length)
                    { 
        ...
        private string ReCreateParts(UriComponents parts, ushort nonCanonical, UriFormat formatAs) 
        {
                ...
                if ((nonCanonical & (ushort)UriComponents.UserInfo) != 0) { 
                    switch (formatAs) { 
                        ...
                        case UriFormat.SafeUnescaped:
                            chars = UnescapeString( m_String, m_Info.Offset.User, m_Info.Offset.Host-1, chars, ref count, '@', '/', '\\', 
                                                    InFact(Flags.UserEscaped)? UnescapeMode.Unescape: UnescapeMode.EscapeUnescape, 
                                                    m_Syntax, false, false);
                            chars[count++] = '@'; 
                            break;

                        case UriFormat.Unescaped:
                            chars = UnescapeString( m_String, m_Info.Offset.User, m_Info.Offset.Host, chars, ref count, c_DummyChar, 
                        ...
                        default: //V1ToStringUnescape 
                            chars = UnescapeString(m_String, m_Info.Offset.User, m_Info.Offset.Host, chars, ref count, c_DummyChar,
                                                    c_DummyChar, c_DummyChar, UnescapeMode.CopyOnly, m_Syntax, false, false);
                            break;
                    } 
                }
                else { 
                    UnescapeString(m_String, m_Info.Offset.User, m_Info.Offset.Host, chars, ref count, 
                        c_DummyChar, c_DummyChar, c_DummyChar, UnescapeMode.CopyOnly, m_Syntax, false, false);
                } 
            ...
            if ((parts & UriComponents.Host) != 0 && stemp.Length != 0) 
            {
                ...
                chars = UnescapeString(stemp, 0, stemp.Length, chars, ref count, '/', '?', '#', mode, m_Syntax, false, false);
                ...
                if ((nonCanonical & (ushort)UriComponents.Query) != 0)
                {
                    ...
                    case UriFormat.UriEscaped:
                        ...
                        else 
                        {
                            ...
                            UnescapeString(m_String, delimiterAwareIndex, m_Info.Offset.Fragment, chars, ref count,
                                c_DummyChar, c_DummyChar, c_DummyChar, UnescapeMode.CopyOnly, m_Syntax, true, false); 
                        } 
                        break;
                    ...
                    case UriFormat.Unescaped:
                        ...
                        chars = UnescapeString(m_String, delimiterAwareIndex, m_Info.Offset.Fragment, chars, ref count, '#', c_DummyChar, c_DummyChar,
                                               (UnescapeMode.Unescape| UnescapeMode.UnescapeAll), m_Syntax, true, false);
                        break;
 
                    default: // UriFormat.SafeUnescaped
 
                        chars = UnescapeString(m_String, delimiterAwareIndex, m_Info.Offset.Fragment, chars, ref count, '#', c_DummyChar, c_DummyChar, 
                                               (InFact(Flags.UserEscaped) ? UnescapeMode.Unescape : UnescapeMode.EscapeUnescape), m_Syntax, true, false);
                        break; 
                    }
                }
                else
                { 
                    UnescapeString( m_String, delimiterAwareIndex, m_Info.Offset.Fragment, chars, ref count, c_DummyChar, c_DummyChar, c_DummyChar,
                                    UnescapeMode.CopyOnly, m_Syntax, true, false); 
                } 
            }
                ...
                { 
                    ...
                    case UriFormat.UriEscaped:
                            ...
                            { 
                                ...
                                UnescapeString( m_String, delimiterAwareIndex, m_Info.Offset.End, chars, ref count, c_DummyChar, c_DummyChar, c_DummyChar,
                                                UnescapeMode.CopyOnly, m_Syntax, false, false);
                            }
                            break; 
                    ...
                    case V1ToStringUnescape: 
                        ...
                        chars = UnescapeString(m_String, delimiterAwareIndex, m_Info.Offset.End, chars, ref count, '#', c_DummyChar, c_DummyChar,
                                               (InFact(Flags.UserEscaped) ? UnescapeMode.Unescape : UnescapeMode.EscapeUnescape) | UnescapeMode.V1ToStringFlag, m_Syntax, false, false); 
                        break;
                    case UriFormat.Unescaped:

                        chars = UnescapeString(m_String, delimiterAwareIndex, m_Info.Offset.End, chars, ref count, '#', c_DummyChar, c_DummyChar, 
                                               UnescapeMode.Unescape | UnescapeMode.UnescapeAll, m_Syntax, false, false);
                        break; 
 
                    default: // UriFormat.SafeUnescaped
 
                        chars = UnescapeString(m_String, delimiterAwareIndex, m_Info.Offset.End, chars, ref count, '#', c_DummyChar, c_DummyChar,
                                               (InFact(Flags.UserEscaped) ? UnescapeMode.Unescape : UnescapeMode.EscapeUnescape), m_Syntax, false, false);
                        break;
                    } 
                }
                else 
        ...
        private unsafe void ParseRemaining() { 
            ...
            fixed (char* str = m_String){
                ...
                if (IsImplicitFile){
                    ...
                    if (((m_Flags & Flags.AuthorityFound) != 0) && (idx + i + 3 >= length || str[idx + i + 1] != '/' || str[idx + i + 2] != '/')){ 
                        cF |= Flags.SchemeNotCanonical;
                    } 
                }
            ...
            fixed (char* str = m_String){ 
                if (IsImplicitFile || ((syntaxFlags & (UriSyntaxFlags.MayHaveQuery | UriSyntaxFlags.MayHaveFragment)) == 0)){
                    ...
                    result = CheckCanonical(str, ref idx, length, (((syntaxFlags & UriSyntaxFlags.MayHaveQuery) != 0) ? '?' : m_Syntax.InFact(UriSyntaxFlags.MayHaveFragment) ? '#' : c_EOL));
                } 
            ...
            if (IsDosPath || (((m_Flags & Flags.AuthorityFound) != 0) && ((syntaxFlags & (UriSyntaxFlags.CompressPath | UriSyntaxFlags.ConvertPathSlashes | UriSyntaxFlags.UnEscapeDotsAndSlashes)) != 0)))
            {
                if (((syntaxFlags & (UriSyntaxFlags.UnEscapeDotsAndSlashes)) != 0) && (result & Check.DotSlashEscaped) != 0){
                ...
                if (((syntaxFlags & (UriSyntaxFlags.CompressPath)) != 0) && ((cF & Flags.E_PathNotCanonical) != 0 || (result & Check.DotSlashAttn) != 0)) 
            ...
            if ((result & Check.DisplayCanonical) == 0){
                ...
                if (((m_Flags & Flags.ImplicitFile) == 0) || ((m_Flags & Flags.UserEscaped) != 0) || (result & Check.ReservedFound) != 0){ 
           ...
           if (buildIriStringFromPath){
            ...
            fixed (char* str = m_String){
                if (idx < length && str[idx] == '?'){
                    ...
                    result = CheckCanonical(str, ref idx, length, ((syntaxFlags & (UriSyntaxFlags.MayHaveFragment)) != 0) ? '#' : c_EOL); 
                    if ((result & Check.DisplayCanonical) == 0){
        ...
        private unsafe ushort CheckAuthorityHelper( char* pString, ushort idx, ushort length, 
                                                    ref ParsingError err, ref Flags flags, UriParser syntax, 
                                                    ref string newHost )
        { 
            int end = length;
            char ch;
           ...
           DomainNameHelper.IsValid(pString, start, ref end, ref dnsNotCanonical, StaticNotAny(flags, Flags.ImplicitFile))) 
            ...
            else if ((iriParsing || (s_IdnScope != UriIdnScope.None)) && ((syntaxFlags & UriSyntaxFlags.AllowDnsHost) != 0) && 
                    ((iriParsing && hostNotUnicodeNormalized) || AllowIdnStatic(syntax, flags)) &&
                    DomainNameHelper.IsValidByIri(  pString, start, ref end, ref dnsNotCanonical, 
                                                    StaticNotAny(flags, Flags.ImplicitFile)))
            {
                CheckAuthorityHelperHandleDnsIri(pString, start, end, startInput, iriParsing, hasUnicode, syntax, userInfoString,
                                                ref flags, ref justNormalized, ref newHost, ref err); 

            } 
#if !PLATFORM_UNIX 
            else if ((s_IdnScope == UriIdnScope.None) && (!s_IriParsing) && ((syntaxFlags & UriSyntaxFlags.AllowUncHost) != 0))
            { 
                //
        ...
        unsafe void CheckAuthorityHelperHandleDnsIri( char* pString, ushort start, int end, int startInput, 
                                            bool iriParsing, bool hasUnicode, UriParser syntax, string userInfoString,
                                            ref Flags flags, ref bool justNormalized, ref string newHost, ref ParsingError err) 
        {
        ...
        { 
            ...
            { 
                ...
                if (AllowIdnStatic(syntax, flags))
                {
                    ...
                    string UniEquvlt = DomainNameHelper.UnicodeEquivalent(pString, startInput, end, ref allAscii, ref atLeastOneIdn); 
        ...
        private unsafe char[] GetCanonicalPath(char[] dest, ref int pos, UriFormat formatAs)
        {
            ...
            if (formatAs != UriFormat.UriEscaped && InFact(Flags.PathNotCanonical))
            {
                ...
                {
                    dest = UnescapeString(pdest, pos, end, dest, ref pos, '?', '#', c_DummyChar, mode, m_Syntax, false, false); 
                }
            }
            else
            { 
        ...
        protected virtual string Unescape(string path) {
            ...
            int count = 0;
            dest = UnescapeString(path, 0, path.Length, dest, ref count, c_DummyChar, c_DummyChar, c_DummyChar, UnescapeMode.Unescape | UnescapeMode.UnescapeAll, null, false, true); 
