...
namespace System.IO {
    ...
    public static class Path
    {
        ...
        internal static String NormalizePath(String path, bool fullCheck) { 
            ...
            // NOTE: Path.NormalizePath is called while resolving security policy
            // early on when a managed app is run. We can't call Environment.IsWin9x() 
            // here which ends up P/Invoking, as P/Invoking here loads the com subsystem 
            // and ends up initializing COM apratment thread state before running any
            // user code. This is wrong. We will get an assert while building 
            // the toolbox folder in this case. To counter this, we are using
            // a FCall method to figure out whether or not we are on 9x
...
#if !PLATFORM_UNIX
/* SSS_WARNINGS_OFF */                    // For short file names, we must try to expand each of them as 
...
/* SSS_WARNINGS_ON */                    if (mightBeShortFileName) { 
        ...
        { 
            ...
            if (r >= MaxPath)
                throw new PathTooLongException(Environment.GetResourceString("IO.PathTooLong")); 
            if (r == 0) {
/* SSS_WARNINGS_OFF */                // Note: GetLongPathName will return ERROR_INVALID_FUNCTION on a 
...
/* SSS_WARNINGS_ON */                    if (mightBeShortFileName) { 
            ...
            if (fullCheck) {
                // NOTE: Return value from GetFullPathName is in TCHARs which means on Win9x it is byte length 
                // Win32 GetFullPathName requires the input buffer to be big enough to fit the initial
                // path which is a concat of CWD and the relative path, this can be of an arbitrary 
