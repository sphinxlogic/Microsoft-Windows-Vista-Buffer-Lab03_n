...
namespace System.Reflection.Emit {
    ...
    { 
        ...
        internal override void GetJitContext(ref int securityControlFlags, 
                                             ref RuntimeTypeHandle typeOwner, 
                                             ref CompressedStack securityContext)
        { 
            if (m_method.m_restrictedSkipVisibility)
                securityControlFlags = (int)SecurityControlFlags.RestrictedSkipVisibilityChecks;
            else if (m_method.m_skipVisibility)
                securityControlFlags = (int)SecurityControlFlags.SkipVisibilityChecks; 
            else
                securityControlFlags = (int)SecurityControlFlags.Default; 
            typeOwner = (m_method.m_typeOwner != null) ? m_method.m_typeOwner.TypeHandle 
                                                       : RuntimeTypeHandle.EmptyHandle;
 
            securityContext = m_method.m_creationContext;
        }
        ...
        private int GetMethodToken() 
        {
            ...
            {
                // No slot yet, push out method handle into the scope to obtain one. There's no harm if a race 
        ...
        internal override unsafe void* GetInstantiationInfo(int token) 
        {
            ...
            GenericFieldInfo gfi = m_scope[token] as GenericFieldInfo;
            if (gmi != null) 
            {
            ...
            if (gfi != null)
            { 
                return (void*)gfi.m_context.Value;
            }
            return null;
        } 
