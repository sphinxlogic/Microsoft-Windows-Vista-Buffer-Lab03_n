    ...
    public class BrowserCapabilitiesFactoryBase {
        ...
        protected IDictionary MatchedHeaders {
            get { 
                if (_matchedHeaders == null)
                    lock (_lock) { 
                        if (_matchedHeaders == null) { 
                            _matchedHeaders = Hashtable.Synchronized(new Hashtable(24, StringComparer.OrdinalIgnoreCase));
                            PopulateMatchedHeaders(_matchedHeaders); 
                        }
                    }
        ...
        internal static string GetBrowserCapKey(IDictionary headers, HttpRequest request) {
            ...
            foreach(String key in headers.Keys) {
                if (key.Length == 0) { 
                    sb.Append(HttpCapabilitiesEvaluator.GetUserAgent(request));
                } 
                else { 
        ...
        internal HttpBrowserCapabilities GetHttpBrowserCapabilities(HttpRequest request) {
            ...
            values[String.Empty] = HttpCapabilitiesEvaluator.GetUserAgent(request); 
