...
internal class FileUtil { 
    ...
    static private bool IsSuspiciousPhysicalPath(string physicalPath, out bool pathTooLong) { 
        bool isSuspicious;
...
#if UNUSED_CODE 
    internal static bool FileExistsUnderHostingIdentity(String filename) {
        bool exists = false; 
    ...
    internal static bool DirectoryExistsUnderHostingIdentity(String dirname) { 
        bool exists = false;
...
#if UNUSED_CODE 
    static internal int GetFileTime(IntPtr handle, out long fileTime) 
    {
        UnsafeNativeMethods.WIN32_BY_HANDLE_FILE_INFORMATION  data; 
        if (!UnsafeNativeMethods.GetFileInformationByHandle(handle, out data)) {
            fileTime = -1;
            return Marshal.GetLastWin32Error();
        } 

        fileTime = (((long)data.ftCreationTimeHigh) << 32) | (long)data.ftCreationTimeLow; 
