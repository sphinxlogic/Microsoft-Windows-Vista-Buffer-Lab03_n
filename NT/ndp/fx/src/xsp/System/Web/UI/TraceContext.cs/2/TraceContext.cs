    ...
    public sealed
        ...
        void Write(string category, string message, Exception errorInfo, bool isWarning, bool writeToDiagnostics) {
            ...
            lock(this) {
                ...
                if (_lastTime != -1) {
                    row[SR.Trace_From_Last] = (((double) (messagetime - _lastTime))/Counter.Frequency).ToString("0.000000", CultureInfo.CurrentCulture); 
                }
        ...
        private void InitRequest() { 
            ...
            for (i = 0; i < n; i++) { 
                ...
                row = NewRow(_requestData, SR.Trace_Response_Headers_Collection);
                row[SR.Trace_Name] = h.Name; 
                row[SR.Trace_Value] = h.Value;
                AddRow(_requestData, SR.Trace_Response_Headers_Collection, row);
            }
