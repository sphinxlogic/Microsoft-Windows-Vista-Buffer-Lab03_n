...
public abstract class TemplateParser : BaseParser, IAssemblyDependencyParser {
    ...
    private void ProcessCodeFile(VirtualPath codeFileVirtualPath) { 
        ...
        // If we're using code besides, the base type is driven by the code file and not by the aspx file.
        // Hence, we never want to use anything but the default base type.  But we can end up with a
        // non-default base type if it was specified in web.config as a basePageType. 
        // Here, we always restore it to is original default value (DevDiv Bugs 129895)
    ...
    private void ProcessCodeBlock(Match match, CodeBlockType blockType, string text) { 
        ...
        if (!PageParserFilterProcessedCodeBlock(CodeConstructTypeFromCodeBlockType(blockType), code, lineNumber)) { 
            ...
            subBuilder = new CodeBlockBuilder(blockType, code, lineNumber, column, CurrentVirtualPath); 
     ...
     */
    private string ProcessAttributes(Match match, out ParsedAttributeCollection attribs, 
            ...
            if (attribName != null) { 
                ...
                try {
                    ...
                    if (fDirective && directiveName.Length > 0 && filter.Length > 0) {
                        ProcessError(SR.GetString( 
                            SR.Device_unsupported_in_directive, directiveName)); 
