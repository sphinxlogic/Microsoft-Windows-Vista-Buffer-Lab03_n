...
public sealed class PageParser : TemplateControlParser { 
    ...
    public static IHttpHandler GetCompiledPageInstance(string virtualPath,
        ...
        if (inputFile != null) {
            // This call temporarily replaces _theHostingEnvironment._virtualPathProvider, and it can get 
            // corrupted if two threads do it at once.  Use a lock to prevent this (VSWhidbey 598332)
            lock (s_lock) {
        ...
        case "enablesessionstate":
            ...
            if (Util.IsFalseString(value)) {
                ...
                ProcessError(SR.GetString(
                    SR.Enablesessionstate_must_be_true_false_or_readonly)); 
            }
    ...
    internal override void PostProcessMainDirectiveAttributes(IDictionary parseData) {
        ...
        if (!flags[buffer] && _errorPage != null) { 
            ProcessError(SR.GetString(
                SR.Error_page_not_supported_when_buffering_off)); 
            return; 
        }
