...
{ 
    /// <summary>
    /// Summary description for DataGridViewAddColumnDialog. 
    /// </summary> 
    ...
    { 
        ...
        { 
            this.cancelButton.Text = SR.GetString(SR.DataGridView_Close); 
        ...
        { 
            if (columns.Contains(name))
            {
                errorString = SR.GetString(SR.DataGridViewDuplicateColumnName, name);
                return false; 
            }
            ...
            if (container != null && container.Components[name] != null) 
            {
                if (!allowDuplicateNameInLiveColumnCollection || liveColumns == null || !liveColumns.Contains(name)) 
                {
                    errorString = SR.GetString(SR.DesignerHostDuplicateName, name);
                    return false;
                } 
            }
            ...
            if (nameCreationService != null && !nameCreationService.IsValidName(name)) 
            {
                if (!allowDuplicateNameInLiveColumnCollection || liveColumns == null || !liveColumns.Contains(name)) 
                {
                    errorString = SR.GetString(SR.CodeDomDesignerLoaderInvalidIdentifier, name);
                    return false;
                } 
            }
