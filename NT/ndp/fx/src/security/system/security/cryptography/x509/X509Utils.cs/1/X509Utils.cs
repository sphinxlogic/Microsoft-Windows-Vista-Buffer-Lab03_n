...
namespace System.Security.Cryptography.X509Certificates {
    ...
    internal class X509Utils {
        ...
        internal static string FindOidInfo(uint keyType, string keyValue, OidGroup oidGroup) {
            Debug.Assert(OidGroup.AllGroups <= oidGroup && oidGroup <= OidGroup.Template, "Invalid OID group"); 
            ...
            // if we couldn't find the OID within a specific group, retry with the generic group
            if(pOidInfo.pszOID == null && oidGroup != OidGroup.AllGroups)
                pOidInfo = CAPI.CryptFindOIDInfo(keyType, pvKey, OidGroup.AllGroups);
