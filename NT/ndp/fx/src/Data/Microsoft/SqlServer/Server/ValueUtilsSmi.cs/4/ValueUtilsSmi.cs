...
namespace Microsoft.SqlServer.Server {
    ...
    // 
...
#if WINFS_UTC
        public SqlUtcDateTime GetSqlUtcDateTime( SmiEventSink_Default sink, ITypedGettersV3 getters, int ordinal, SmiMetaData metaData ) {
            return GetUdt( sink, getters, ordinal, typeof( SqlUtcDateTime ) );
        } 

        public SqlDate GetSqlDate( SmiEventSink_Default sink, ITypedGettersV3 getters, int ordinal, SmiMetaData metaData ) { 
            return GetUdt( sink, getters, ordinal, typeof( SqlDate ) ); 
        }
 
        public SqlTime GetSqlTime( SmiEventSink_Default sink, ITypedGettersV3 getters, int ordinal, SmiMetaData metaData ) {
            return GetUdt( sink, getters, ordinal, typeof( SqlTime ) );
        }
#endif 
        ...
        private static void GetNullOutputParameterSmi(SmiMetaData metaData, SqlBuffer targetBuffer, ref object result) { 
            if (SqlDbType.Udt == metaData.SqlDbType) { 
                ...
                SqlBuffer.StorageType stype = __dbTypeToStorageType[unchecked((int)metaData.SqlDbType)];
...
#if WINFS_UTC
        internal static void SetSqlUtcDateTime( SmiEventSink_Default sink, ITypedSettersV3 setters, int ordinal, SmiMetaData metaData, SqlUtcDateTime value ) {
            ThrowIfInvalidSetterAccess( metaData, ExtendedClrTypeCode.SqlUtcDateTime ); 

            SetSqlUtcDateTime_Unchecked( sink, setters, ordinal, value ); 
        } 

        internal static void SetSqlDate( SmiEventSink_Default sink, ITypedSettersV3 setters, int ordinal, SmiMetaData metaData, SqlDate value ) { 
            ThrowIfInvalidSetterAccess( metaData, ExtendedClrTypeCode.SqlDate );

            SetSqlDate_Unchecked( sink, setters, ordinal, value );
        } 

        internal static void SetSqlTime( SmiEventSink_Default sink, ITypedSettersV3 setters, int ordinal, SmiMetaData metaData, SqlTime value ) { 
            ThrowIfInvalidSetterAccess( metaData, ExtendedClrTypeCode.SqlTime ); 

            SetSqlTime_Unchecked( sink, setters, ordinal, value ); 
        }
#endif
    ...
    // 
        ...
        // This method is almost the same as SetBytes_FromReader and the only difference is we call GetBytesInternal intead of GetBytes on the reader (SqlDataReader in this case). 
        // It is used to read Xml bytes in chunks from a reader for better performance.
        private static void SetBytes_FromSqlReader(SmiEventSink_Default sink, SmiTypedGetterSetter setters, int ordinal, SmiMetaData metaData, SqlDataReader reader, int offset) { 
            ...
            while ((length < 0  || lengthWritten < length) && 
                    0 != (bytesRead = reader.GetBytesInternal(ordinal, currentOffset, buffer, 0, chunkSize)) &&
                    0 != bytesWritten) { 
                bytesWritten = setters.SetBytes(sink, ordinal, currentOffset, buffer, 0, checked((int) bytesRead)); 
                sink.ProcessMessagesAndThrow();
                checked{ currentOffset += bytesWritten; } 
...
#if WINFS_UTC
        private static void SetSqlUtcDateTime_LengthChecked( SmiEventSink_Default sink, ITypedSettersV3 setters, int ordinal, SqlUtcDateTime value ) {
            SetUdt_LengthChecked( sink, setters, ordinal, typeof( SqlUtcDateTime ) ); 
        }
 
        private static void SetSqlDate_LengthChecked( SmiEventSink_Default sink, ITypedSettersV3 setters, int ordinal, SqlDate value ) { 
            SetUdt_LengthChecked( sink, setters, ordinal, typeof( SqlDate ) );
        } 

        private static void SetSqlTime_LengthChecked( SmiEventSink_Default sink, ITypedSettersV3 setters, int ordinal, SqlTime value ) {
            SetUdt_LengthChecked( sink, setters, ordinal, typeof( SqlTime ) );
        } 
#endif
    ...
    // 
        ...
        private static bool CanAccessGetterDirectly( SmiMetaData metaData, ExtendedClrTypeCode setterTypeCode ) {
            ...
            Debug.Assert( ExtendedClrTypeCode.Boolean <= setterTypeCode && ExtendedClrTypeCode.DateTimeOffset >= setterTypeCode ); 
