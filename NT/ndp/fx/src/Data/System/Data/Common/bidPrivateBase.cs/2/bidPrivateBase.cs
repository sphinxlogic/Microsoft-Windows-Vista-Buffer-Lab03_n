...
//----------------------------------------------------------------------------------------------- 
#define UNUSED_CODE     // tag for commenting out unused code
...
//
//  In V1/V1.1 compatibility mode, an artificial inheritace is used in order to to support
//  this model. Every module (or single-module assembly) must use a private copy of 
//
//      internal sealed class Bid : BidPrivateBase 
// 
//  implemented in assembly (module) specific file <Assembly_Name>_BID.cs
// 
//  NOTE:
...
[ComVisible(false)]
internal class BidPrivateBase 
{
    //
    //  Loader Stub DLL. Can be the assembly itself (Visual Studo 2005 mixed mode dll).
    // 
    protected const string dllName = "BidLdr.dll";
    ...
    internal static bool StateDumpOn { 
        get { return (modFlags & ApiGroup.StateDump) != 0; } 
    }
    ...
    { 
       ...
       #else
        Module mod  = Assembly.GetExecutingAssembly().GetModules(false)[0];
       #endif 
    ...
    { 
        ...
        [DllImport(dllName, CharSet=CharSet.Unicode, CallingConvention=CallingConvention.Cdecl, 
        EntryPoint="DllBidTraceCW")] extern 
        internal static void TraceBin(IntPtr hID, UIntPtr src, UIntPtr info, string fmtPrintfW, byte[] buff, UInt16 len);
       ...
       #else 
        [DllImport(dllName, CharSet=CharSet.Ansi, EntryPoint="DllBidCtlProc")] extern
       #endif
       ...
       #else
        [DllImport(dllName, CharSet=CharSet.Ansi)] extern
       #endif 
...
internal sealed class SignatureGenerator 
{
    ...
    { 
        ...
        internal int Consider(params object[] args) 
        {
           ...
           #else
            BidPrivateBase.DASSERT( NumOfArgs(idx1) == args.Length ); 
           #endif 
            return idx1;
        } 
        ...
        internal int Count {
            get {
               ...
               #else 
                BidPrivateBase.DASSERT( cnt == _numArgsPool.Count );
               #endif 
                return cnt;
            }
        }
    ...
    internal static void ScopeEnter(out IntPtr hScp, string fmt, params object[] args) 
    {
       ...
       #else
        BidPrivateBase.ScopeEnter(out hScp, "%s", buf);
       #endif 
    ...
    private sealed class Buckets
    {
        ...
        private string getModuleName() 
        {
           ...
           #else
            modName  = Assembly.GetExecutingAssembly().GetModules(false)[0].Name;
           #endif
    ...
    struct Templates 
    {
        internal struct Headers 
        {
            ...
            internal const string File =
            "//-----------------------------------------------------------------------------------\r\n" +
            "// <copyright file=\"^MODNAME^_BID.cs\" company=\"Microsoft\">\r\n" +
            "//     Copyright (c) Microsoft Corporation.  All rights reserved.\r\n" + 
            "// </copyright>\r\n" +
            "//-----------------------------------------------------------------------------------\r\n" + 
            "\r\n" + 
