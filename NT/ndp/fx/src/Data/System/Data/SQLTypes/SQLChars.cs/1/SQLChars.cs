...
namespace System.Data.SqlTypes { 
    ...
    public sealed class SqlChars : System.Data.SqlTypes.INullable, IXmlSerializable, ISerializable { 
		...
		// 1) SqlChars is Null
 		//		- m_pchData and m_stream must be null, m_lCuLen must be x_lNull
		// 2) SqlChars contains a valid buffer,
		//		- m_rgchBuf must not be null, and m_pchData and m_stream must be null 
		// 3) SqlChars contains a valid pointer
 		//		- m_pchData must not be null, m_rgchBuf could be null or not, 
...
#pragma warning disable 169 
        ...
        // however, it can not be removed or it will break serialization with pre-release builds of V2.0
 		private  IntPtr	            m_pchData;	// Pointer to data in unmanaged memory, We don't own this memory, should never free
#pragma warning restore 169 
	...
	public SqlChars(SqlString value) : this (value.IsNull ? (char[])null : value.Value.ToCharArray()) {
...
#if WINFSFunctionality 
        internal SqlChars(byte[] cookie): this((char[]) null) {
        ...
        public 
#endif 
        // Internal for WinFS until UDTExtensions rebuilt against DWPriv.dll
        bool IsFilled { 
            get {
        ...
        internal void Fill(SqlConnection connection, SqlTransaction transaction) { 
            ...
            Object result = SQLUtility.GetLOBFromCookie(m_cookie, connection, transaction); 
            m_cookie = null;
 
            if ((result == null) || (result is DBNull))
                SetNull();
            else
                SetBuffer(((string)result).ToCharArray()); 
        }
#endif 
...
#if WINFSFunctionality 
		// User is not allowed to use any methods until this SqlChars has been filled 
 		// except Fill()
