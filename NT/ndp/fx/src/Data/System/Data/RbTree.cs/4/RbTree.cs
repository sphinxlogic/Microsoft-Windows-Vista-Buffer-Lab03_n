...
//      IndexOutOfRange                             = 12, 
    ...
    internal abstract class RBTree<K> : System.Collections.IEnumerable { 
        ...
        private int RBDeleteX(int root_id, int z_id, int mainTreeNodeID)
        {
            ...
            else if (y_id == Left (py_id))    // patch y's parent to point to X as its child
                SetLeft (py_id, x_id); 
            else
                SetRight (py_id, x_id);
            ...
            if (Next(mNode) != NIL) 
            {
                // patch mNode to point to satellite tree root and have the same key value. 
            ...
            { 
                ...
                // patch a next reference to z_id (if any) 
