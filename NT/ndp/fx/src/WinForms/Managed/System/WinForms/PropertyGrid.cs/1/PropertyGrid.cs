        ...
        protected override void WndProc(ref Message m) {
            ...
            switch (m.Msg) {
                ...
                case AutomationMessages.PGM_GETBUTTONSTATE: 
                    if (toolStrip != null) { 
                        ToolStripButton button = toolStrip.Items[unchecked((int)(long)m.WParam)] as ToolStripButton;
                        if (button != null) { 
                ...
                case AutomationMessages.PGM_GETBUTTONTOOLTIPTEXT: 
                    if (toolStrip != null) {
                        ...
                        if (m.Msg == AutomationMessages.PGM_GETBUTTONTEXT) {
                            t = toolStrip.Items[unchecked((int)(long)m.WParam)].Text; 
                        }
                        else { 
                            t = toolStrip.Items[unchecked((int)(long)m.WParam)].ToolTipText; 
                        }
                ...
                case AutomationMessages.PGM_GETTESTINGINFO: { 
                    ...
                    int len = testingInfo.Length; 

                    // If caller provides return buffer, marhsal back null-terminated copy of string 
                    IntPtr ptr = m.LParam;
                    if (ptr != IntPtr.Zero) {
                        for (int i = 0; i < len; ++i)
                            Marshal.WriteInt16((IntPtr) ((long) ptr + (i * 2)), (short) testingInfo[i]); 

                        Marshal.WriteInt16((IntPtr) ((long) ptr + (len * 2)), 0); 
                    } 

                    // Always return the string length (allows caller to query for length first, 
                    // by specifying null return buffer, so they can pre-allocate enough space).
                    m.Result = (IntPtr) len;
                    return;
                    } 
