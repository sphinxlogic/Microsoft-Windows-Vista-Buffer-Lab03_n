                ...
                { 
                    ...
                    { 
                        if (dataGridViewCell.SwitchFormattedValue())
                        {
                            ...
                            int visibleRowIndex = dataGridView.Rows.GetRowCount(DataGridViewElementStates.Visible, 0, dataGridViewCell.RowIndex); 
                            int visibleColumnIndex = dataGridView.Columns.ColumnIndexToActualDisplayIndex(dataGridViewCell.ColumnIndex, DataGridViewElementStates.Visible);
                            bool columnHeadersVisible = dataGridView.ColumnHeadersVisible; 
                            bool rowHeadersVisible = dataGridView.RowHeadersVisible;

                            int objectID = visibleRowIndex +
                                           (columnHeadersVisible ? 1:0) // + 1 because the top header row acc obj is at index 0 
                                           + 1;                         // + 1 because objectID's need to be positive and non-zero
                            int childID = visibleColumnIndex + 
                                          (rowHeadersVisible ? 1:0);    // + 1 because the row header cell is at index 0 in row header row acc obj 

                            Control.ControlAccessibleObject controlAccObj = (Control.ControlAccessibleObject)dataGridView.AccessibilityObject; 
                            controlAccObj.NotifyClients(AccessibleEvents.DefaultActionChange, objectID, childID);
                        }
                        if (endEditMode)
                        { 
