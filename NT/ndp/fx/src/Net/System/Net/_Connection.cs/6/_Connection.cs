...
#endif 
#if TRAVE 
            GlobalLog.Print("Connection#" + ValidationHelper.HashString(this) + "::Finalize() MyLocalPort:" + MyLocalPort + " disassociating ");
#endif 
            ...
            try 
            {
                try { 
                    ...
                    if (request.Address.Scheme == Uri.UriSchemeHttps) {
                        TlsStream tlsStream = new TlsStream(request.Address.Host, NetworkStream, request.ClientCertificates, ServicePoint, request, request.Async ? request.GetConnectingContext().ContextCopy : null); 
                    ...
                    // 
                    // There is a race with Abort so TlsStream ctor may throw. 
...
quit:
                        ...
                        else if (parseSubStatus == DataParseStatus.Done) 
                        {
                            ...
                            if (m_ResponseData.m_StatusCode == HttpStatusCode.Continue || m_ResponseData.m_StatusCode == HttpStatusCode.BadRequest) {
                                ...
                                if (m_ResponseData.m_StatusCode == HttpStatusCode.BadRequest) {
                                    ...
                                    //if we got a 100continue we ignore it and start looking for a final response
...
#if TRAVE 
                foreach (HttpWebRequest request in m_WaitList) {
                    GlobalLog.Print("Request removed from WaitList#"+ValidationHelper.HashString(request)); 
                } 
        ...
        { 
            ...
            { 
                ...
                { 
                    // Tell the request's SubmitWriteStream that the connection will be closed.  It should ignore any 
            ...
            // 
            // Any exception is processed by HandleError() and ignored to avoid throwing on a thread pool
