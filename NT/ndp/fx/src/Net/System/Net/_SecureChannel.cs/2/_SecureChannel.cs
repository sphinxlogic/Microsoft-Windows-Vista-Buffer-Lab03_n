...
namespace System.Net.Security { 
    ...
    internal class SecureChannel {
        ...
        internal SecureChannel(string hostname, bool serverMode, SchProtocols protocolFlags, X509Certificate serverCertificate, X509CertificateCollection clientCertificates, bool remoteCertRequired, bool checkCertName, bool checkCertRevocationStatus, LocalCertSelectionCallback certSelectionDelegate) 
        {
            ...
            if (Logging.On) Logging.PrintInfo(Logging.Web, this, ".ctor", "hostname=" + hostname + ", #clientCertificates=" + ((clientCertificates == null) ? "0" : clientCertificates.Count.ToString(NumberFormatInfo.InvariantInfo)));
        ...
        private bool AcquireClientCredentials(ref byte[] thumbPrint) 
        {
            ...
            try {
                ...
                {
                    SecureCredential secureCredential = new SecureCredential(SecureCredential.CurrentVersion, selectedCert, SecureCredential.Flags.ValidateManual | SecureCredential.Flags.NoDefaultCred, m_ProtocolFlags); 
        ...
        { 
            ...
            try { 
                ...
                { 
                    SecureCredential secureCredential = new SecureCredential(SecureCredential.CurrentVersion, selectedCert, SecureCredential.Flags.Zero, m_ProtocolFlags);
        ...
        internal SecurityStatus Encrypt(byte[] buffer, int offset, int size, ref byte[] output, out int resultSize) { 
            ...
            if (errorCode != 0) {
                ...
                resultSize = size + m_HeaderSize + securityBuffer[2].size;  // The trailer may not be consumed at full 
        ...
        internal SecurityStatus Decrypt(byte[] payload, ref int offset, ref int count) {
            ...
            int errorCode = SSPIWrapper.DecryptMessage(GlobalSSPI.SSPISecureChannel, m_SecurityContext, decspc, 0);
 
            count = 0;

            BufferType bufferType = BufferType.Data;
            if ((SecurityStatus)errorCode != SecurityStatus.OK) 
                bufferType = BufferType.Extra;
 
            //The sorta hack is that we use the data size from returned buffer and apply the value to the original buffer 
            for (int i = 0; i < decspc.Length; ++i) {
                if (decspc[i].type == bufferType) { 
                    count = decspc[i].size;
                    break;
                }
            } 
            if (bufferType == BufferType.Data && count > 0)
            { 
                offset+=m_HeaderSize; 
            }
            return (SecurityStatus) errorCode; 
        ...
        { 
            ...
            try {
                ...
                else 
                {
                    ...
                    chain.Build(remoteCertificateEx);
