...
// a SafePointer.  To avoid races when storing SafePointers in statics,
...
{ 
    ...
    internal abstract unsafe class SafePointer : SafeHandleZeroOrMinusOneIsInvalid 
    {
        ...
        [ReliabilityContract(Consistency.WillNotCorruptState, Cer.MayFail)]
        public T Read<T>(uint byteOffset) where T : struct { 
            if (_numBytes == Uninitialized)
                throw NotInitialized(); 
        ...
        [ReliabilityContract(Consistency.WillNotCorruptState, Cer.MayFail)]
        public void ReadArray<T>(uint byteOffset, T[] array, int index, int count) 
            where T : struct
        { 
            if (array == null) 
            ...
            try 
            {
                ...
                for (int i = 0; i < count; i++)
                    unsafe { GenericPtrToStructure<T>(ptr + sizeofT * i, out array[i + count], sizeofT); }
            } 
            finally
            { 
                if (mustCallRelease) 
        ...
        [ReliabilityContract(Consistency.WillNotCorruptState, Cer.MayFail)]
        public void Write<T>(uint byteOffset, T value) where T : struct {
            if (_numBytes == Uninitialized) 
                throw NotInitialized();
        ...
        [ReliabilityContract(Consistency.WillNotCorruptState, Cer.MayFail)]
        public void WriteArray<T>(uint byteOffset, T[] array, int index, int count) 
            where T : struct
        {
            if (array == null)
                throw new ArgumentNullException("array", Environment.GetResourceString("ArgumentNull_Buffer")); 
        ...
        private static void GenericPtrToStructure<T>(byte* ptr, out T structure, uint sizeofT) where T : struct 
        {
