                    ...
                    fixed (int *T = s_T) {
                        fixed (int *TF = s_TF) { 
                            ...
                            for (int blockNum = 0;  blockNum < iNumBlocks; ++blockNum) { 
                                if (m_cipherMode == CipherMode.CFB) { 
                                    Utils.BlockCopy(m_shiftRegister, 0, work, 0, m_blockSizeBytes);
                                } else { 
                                    if (blockNum != iNumBlocks - 1 || padSize == 0) {
                                        Utils.BlockCopy(inputBuffer, workBaseIndex, work, 0, m_blockSizeBytes);
                                    } else {
                                        int padIndex = 0; 
                                        index = workBaseIndex;
                                        for (int i = 0; i < m_Nb; ++i) { 
                                ...
                                if (m_cipherMode == CipherMode.CFB) { 
                                    ...
                                    if (m_cipherMode == CipherMode.CBC) { 
                                        Utils.BlockCopy(temp, 0, m_lastBlockBuffer, 0, m_blockSizeBytes); 
                                    }
                                } 
