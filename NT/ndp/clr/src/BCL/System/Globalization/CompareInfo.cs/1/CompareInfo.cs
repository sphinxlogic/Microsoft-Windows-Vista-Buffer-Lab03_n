...
#region Serialization 
        ...
        internal int GetSortingLCID(int culture) {
            ...
            // Note we cannot use IsSynthetic property directly here because CultureTableRecord is not
            // constructed yet and it depends on the sort Id we return here in the construction. 
            if (cultureObj.m_cultureTableRecord.IsSynthetic)
            {
                return culture;
            } 
        ...
        [NonSerialized] static int fFindNLSStringSupported;   // Defaults to 0 -- means valid, or not yet checked
        ...
        {
            BCLDebug.Assert(startIndex + 1 >= length, "[CompareInfo.SyntheticLastIndexOf] invalid parameters.");
            if(fFindNLSStringSupported >= 0) 
            {
                int retval = FindNLSStringWrap(m_sortingLCID, nativeCompareFlags | Win32Native.FIND_FROMEND, source, startIndex - length + 1, length, value, value.Length); 
                if(retval > -1) 
                {
                    return(retval + (startIndex - length + 1)); 
                }
                else if(retval == -1)
                {
                    return(retval); 
            ...
            for (int i=0; i<length; i++)
            { 
                int index = SyntheticIsSuffix(source, startIndex - i, length - i, value, nativeCompareFlags);
...
#if !FEATURE_PAL
            // <SyntheticSupport/> 
            if (IsSynthetic) { return new SortKey(this.m_sortingLCID, source, options); }
#endif // !FEATURE_PAL 
...
#if !FEATURE_PAL
            // <SyntheticSupport/> 
            if (IsSynthetic) { return new SortKey(this.m_sortingLCID, source, CompareOptions.None); }
#endif // !FEATURE_PAL
        ...
        unsafe internal int GetHashCodeOfString(string source, CompareOptions options) 
        {
            ...
            // <SyntheticSupport/>
            if (IsSynthetic) { return CultureInfo.InvariantCulture.CompareInfo.GetHashCodeOfString(source, options); }
#endif // !FEATURE_PAL 
