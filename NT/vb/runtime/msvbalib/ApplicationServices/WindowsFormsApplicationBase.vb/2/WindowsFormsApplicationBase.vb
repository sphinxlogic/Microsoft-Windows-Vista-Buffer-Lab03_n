...
#Region "Application model Event Delegates and Event Argument definitions"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    <EditorBrowsable(EditorBrowsableState.Advanced), Runtime.InteropServices.ComVisible(False)> _ 
...
#End Region
    ...
    ''' using the application model in YUKON.   
    ''' </remarks> 
     ...
     Public Class WindowsFormsApplicationBase : Inherits ConsoleApplicationBase 
        ...
        Public Sub New(ByVal authenticationMode As AuthenticationMode)
            ...
            If authenticationMode = authenticationMode.Windows Then 
                Try
                    'call to System.Security.SecurityManager.IsGranted(New SecurityPermission(SecurityPermissionFlag.ControlPrincipal)) 
        ...
        Public Sub Run(ByVal commandLine As String()) 
            'Microsoft.VisualBasic.MsgBox("Attach Debugger") 'uncomment to facilitate debugging single-instance applications 
            ...
            Else 'This is a Single-Instance application
                ...
                If OperatingSystemSupportsAuthentication = True Then 'We can only use an ACL if we have a user (EventWaitHandleAccessRule creates a NTAccount object)
                    ...
                    'Event names are scoped such that two users logged on at the same time will get unique events even though they have the same name. 
                ...
                If WeAreTheFirstInstance Then 
                    ...
                    Try 
                        Dim ServerChannel As TcpChannel = RegisterChannel(OperatingSystemSupportsAuthentication) 'Register the server channel that will listen for incoming connection messages from clients
                ...
                Else '--- We are launching a subsequent instance. 
                    ...
                    RegisterChannel(OperatingSystemSupportsAuthentication) 'register a client channel to communicate with the remote object.  We can't use a default channel because we need authentication turned on to match the server channel we established during the 1st instance 
                    ...
                    PermissionsToDoRemoting.AddPermission(New System.Net.SocketPermission(System.Net.NetworkAccess.Connect, Net.TransportType.Tcp, Me.HostName, System.Net.SocketPermission.AllPorts)) 'I use AllPorts because we don't know which port we will have hooked up on.  We get whatever port was available at runtime 
       ...
       Protected Overridable Sub OnStartupNextInstance(ByVal eventArgs As StartupNextInstanceEventArgs)
        ...
        Protected Overridable Sub OnRun() 
            ...
            Finally
                ...
                AsyncOperationManager.SynchronizationContext = m_AppSyncronizationContext 'Restore the prior [....] context
        ...
        Private Const SECOND_INSTANCE_TIMEOUT As Integer = 1000 'milliseconds.  How long a subsequent instance will wait for the original instance to get on its feet.  This is only useful if the 1st instance went belly up and stranded the semaphore for some reason - we just wouldn't want to hang forever is all. 
        ...
        Private m_FirstInstanceMemoryMappedFileHandle As SafeFileHandle 'Used to communicate the URL of the single instance between processes.  We hang on to it for the life of the original instance so it doesn't get garbage collected and released 
        ...
        Private m_NetworkAvailChangeLock As New Object '[....] object
        ...
        Private m_HostName As String ' The loopback IP Address connecting to the same machine. This can be in IPv4 (127.0.0.1) or IPv6 (0000:0000:0000:0000:0000:0000:0.0.0.1) format. 
        ...
        ''' ;HostName 
        ''' <summary>
        ''' Return the loopback IP Address connecting to the same machine in IPv4 or IPv6 format.
        ''' </summary>
        ''' <returns>A string containing the loop back address</returns> 
        ''' <remarks>
        '''    This property only attempts to detect IPv4 / IPv6 the first time when m_HostName is not initialized. 
        ''' </remarks> 
        Private ReadOnly Property HostName() As String
            Get 
                If String.IsNullOrEmpty(m_HostName) Then
                    If System.Net.Sockets.Socket.SupportsIPv4 Then
                        m_HostName = System.Net.IPAddress.Loopback.ToString()
                    ElseIf System.Net.Sockets.Socket.OSSupportsIPv6 Then 
                        m_HostName = System.Net.IPAddress.IPv6Loopback.ToString()
                    Else 
                        ' No IPv4 or IPv6. Throw exception and quit here (otherwise Socket will throw exception later). 
        ...
        Private Function ReadUrlFromMemoryMappedFile() As String
            ...
            Dim URL As String 
            Using MemoryMappedFileHandle As SafeHandleZeroOrMinusOneIsInvalid = New SafeFileHandle(UnsafeNativeMethods.OpenFileMapping(FILE_MAP_READ, False, m_MemoryMappedID), True) 
                If MemoryMappedFileHandle.IsInvalid Then
                    Return Nothing 
                End If

                Dim StartAddressOfMappedView As System.IntPtr
                Try 
                    StartAddressOfMappedView = UnsafeNativeMethods.MapViewOfFile( _
                        New System.Runtime.InteropServices.HandleRef(Nothing, MemoryMappedFileHandle.DangerousGetHandle), FILE_MAP_READ, 0, 0, 0) 
                    If StartAddressOfMappedView = IntPtr.Zero Then 
                        Throw ExceptionUtils.GetWin32Exception(ResID.MyID.AppModel_CantGetMemoryMappedFile) 'This exception does a get last error and generates the appropriate message
                    End If 

                    URL = System.Runtime.InteropServices.Marshal.PtrToStringUni(StartAddressOfMappedView)
                Finally
                    If StartAddressOfMappedView <> IntPtr.Zero Then 
                        UnsafeNativeMethods.UnmapViewOfFile(New System.Runtime.InteropServices.HandleRef(Nothing, StartAddressOfMappedView)) 'we don't need this anymore so free the memory
                    End If 
                End Try 
            End Using 'Closes the handle when we leave this method
            Return URL 
        End Function
        ...
        Private Sub WriteUrlToMemoryMappedFile(ByVal URL As String) 
            ...
            Const SYSTEM_PAGING_FILE As Integer = &HFFFFFFFF

            Dim PageFileHandle As New System.Runtime.InteropServices.HandleRef(Nothing, CType(SYSTEM_PAGING_FILE, System.IntPtr)) 'doesn't need to be a safe handle--it's a constant, not a handle to an OS resource 

            Using SecurityAttributes As New NativeTypes.SECURITY_ATTRIBUTES 
                SecurityAttributes.bInheritHandle = False 'no inheritance of permissions needed for memory mapped file handles 
                ...
                m_FirstInstanceMemoryMappedFileHandle = New SafeFileHandle(UnsafeNativeMethods.CreateFileMapping( _
                            PageFileHandle, SecurityAttributes, PAGE_READWRITE, 0, (URL.Length + 1) * 2, m_MemoryMappedID), True)
            ...
            Dim StartAddressOfMappedFile As IntPtr
            Try
                StartAddressOfMappedFile = UnsafeNativeMethods.MapViewOfFile( _
                    New System.Runtime.InteropServices.HandleRef(Nothing, m_FirstInstanceMemoryMappedFileHandle.DangerousGetHandle), FILE_MAP_WRITE, 0, 0, 0) 
                If StartAddressOfMappedFile = IntPtr.Zero Then
                    Throw ExceptionUtils.GetWin32Exception(ResID.MyID.AppModel_CantGetMemoryMappedFile) 
                End If 

                Dim UrlChars() As Char = URL.ToCharArray() 
                System.Runtime.InteropServices.Marshal.Copy(UrlChars, 0, StartAddressOfMappedFile, UrlChars.Length)
            Finally
                If StartAddressOfMappedFile <> IntPtr.Zero Then
                    UnsafeNativeMethods.UnmapViewOfFile(New System.Runtime.InteropServices.HandleRef(Nothing, StartAddressOfMappedFile)) 
                    'Even though we've unmapped the view, because we are hanging on to the file handle in a field member (m_FirstInstanceMemoryMappedFileHandle)
                    'we won't lose our memory mapped file until this instance goes down.  Which is fine because at that point there will not be any other instances 
                    'trying to open the memory mapped file. 
                End If
            End Try 
        End Sub
        ...
        ''' <param name="SecureChannel">Whether we can use authentication on the channel or not</param>
        ...
        Private Function RegisterChannel(ByVal SecureChannel As Boolean) As TcpChannel 
            ...
            Dim PermissionsToDoRemoting As New System.Security.PermissionSet(PermissionState.None)
            PermissionsToDoRemoting.AddPermission(New System.Security.Permissions.SecurityPermission(SecurityPermissionFlag.SerializationFormatter Or SecurityPermissionFlag.ControlPrincipal Or SecurityPermissionFlag.UnmanagedCode)) 
            PermissionsToDoRemoting.AddPermission(New System.Net.SocketPermission(Net.NetworkAccess.Accept, Net.TransportType.Tcp, Me.HostName, 0))
            ...
            ChannelProperties.Add("bindTo", Me.HostName) 
            ChannelProperties.Add("port", 0) 'Listen on any open port.
            If SecureChannel = True Then 'Only >= NT support authentication on the server 
            ...
            End If 
            Dim AuthenticatingChannel As New TcpChannel(ChannelProperties, Nothing, Nothing)
            ChannelServices.RegisterChannel(AuthenticatingChannel, false) 
        ...
        Private Class RemoteCommunicator : Inherits System.MarshalByRefObject
            ...
            ''' Note that we don't own the ConnectionMadeSemaphore wait handle so don't
            ''' try to release it in this class.</remarks>
            ...
            ''' </remarks> 
            <System.Runtime.Remoting.Messaging.OneWay()> Public Sub RunNextInstance(ByVal Args As System.Collections.ObjectModel.ReadOnlyCollection(Of String)) 
            ...
            Private m_ConnectionMadeSemaphore As System.Threading.EventWaitHandle
        ...
        Private Sub DoApplicationModel()
            ...
            'Only do the try/catch if we aren't running under the debugger.  If we do try/catch under the debugger the debugger never gets a chance at exceptions which breaks the exception helper
            If Not System.Diagnostics.Debugger.IsAttached Then 
                'NO DEBUGGER ATTACHED - we use a catch so that we can run our UnhandledException code
                'Note - code changes within this IF (that don't pertain to exception handling) need to be mirrored in the ELSE debugger attached clause below 
                Try 
        ...
        Private Function GetApplicationInstanceID(ByVal Entry As Assembly) As String
            ...
            'machine, are separate.  So you can have session 1 running as User1 and have a global system object named "FOO" that won't conflict with 
            'any other global system object named "FOO" whether it be in session 2 running as User1 or session n running as whoever. 
